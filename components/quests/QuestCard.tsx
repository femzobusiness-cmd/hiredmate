...
export function QuestCard({ quest, index, variant = 'daily' }: QuestCardProps) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        'relative overflow-hidden rounded-[20px] border border-border bg-white p-5 shadow-card transition-all hover:-translate-y-1',
        isWeekly
    ...
    >
      {quest.completed && (
        <motion.div
          ...
          className="pointer-events-none absolute right-4 top-4 z-10 flex h-9 w-9 items-center justify-center rounded-full bg-green-500 text-white shadow-md"
        >
